        ...
        case 1:
            ...
            if let detailedModel = model.detailContent {
                ...
                rejectButton.addTarget { _ in
                    BBSJarvis.friendConfirm(messageID: self.model.id, action: "reject", success: { _ in
                ...
                agreeButton.addTarget { _ in
                    BBSJarvis.friendConfirm(messageID: self.model.id, action: "agree", success: { _ in
    ...
    func replyButtonDidTap(sender: UIButton) {
        ...
        editDetailVC.doneBlock = { [weak editDetailVC] string in
            ...
            if cutString.count > 91 {
                shortString = (cutString as NSString).substring(with: NSRange(location: 0, length: 90))
            }
//            let resultString = string + "\n > 回复 #\(detailedModel.floor) \(self.model.authorName): \n" + shortString.replacingOccurrences(of: ">", with: "> >", options: .regularExpression, range: nil)
            ...
            BBSJarvis.reply(threadID: detailedModel.thread_id, content: resultString, toID: self.model.authorId, anonymous: editDetailVC?.isAnonymous ?? false, failure: { _ in
