...
//    var loginBtn = UIButton.borderButton(title: "马上体验")
    ...
    func initUI() {
        ...
        newUserButton.addTarget { _ in
            let vc = InfoModifyController(title: "用户注册", items: ["姓名-输入真实姓名-real_name", "学号-输入学号(新生请输入通知书号)-stunum", "身份证号-身份证号仅用于身份验证-cid", "用户名-2～12个字母-username", "密码-8~16位英文/符号/数字-password-s", "再次确认-再次输入密码-repass-s"], style: .bottom, headerMsg: "欢迎新用户！请填写以下信息", handler: nil)
